...
Separate code from configurations: we can pass configurations (i.e., variables) to modules, allowing them to be reused with different values.

In the example, `main.tf` uses the module we created by calling it and passing the variables required by that module. There's no need for `terraform.tfvars` since all variables are passed directly when the module is called. If some variables are not passed, they might be needed.
